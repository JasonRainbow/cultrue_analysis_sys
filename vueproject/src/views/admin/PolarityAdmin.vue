<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>情感极性分析管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 搜索筛选 -->
    ...
    <el-table size="small" :data="list_data" highlight-current-row v-loading="loading"
      ...
      <el-table-column prop="positive" label="积极评论数" width="100">
      </el-table-column>
      <el-table-column prop="negative" label="消极评论数" width="100">
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="40%" @click="closeDialog">
      <el-form label-width="130px" :model="editForm" :rules="rules" ref="editForm">
        ...
        <el-form-item label="积极评论数" prop="positive">
          <el-input size="small" v-model="editForm.positive" auto-complete="off"
                    placeholder="请输入积极评论数"></el-input>
        </el-form-item>
        <el-form-item label="消极评论数" prop="negative">
          <el-input size="small" v-model="editForm.negative" auto-complete="off"
                    placeholder="请输入消极评论数"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        updateSupport: 0,
        // 上传的地址
        url: "http://localhost:9090" + "/api/polarity/import"
      },
      editForm: {
        id: null,
        workId: null,
  ...
  methods: {
    get_data(param) {
      this.loading = true
      getPolarityByPage(param).then(res => {
        this.loading = false
          ...
          if (this.title === '添加') {
            addPolarity(this.editForm)
              ...
              .catch(err => {
                this.editFormVisible = false
                this.loading = false
                this.$message.error('添加情感极性分析失败，请稍后再试！')
              })
          } else {
            // console.log(this.editForm)
            updatePolarity(this.editForm)
              .then(res => {
                this.editFormVisible = false
                this.loading = false
                if (res.code === "0") {
              ...
              .catch(err => {
                this.editFormVisible = false
                this.loading = false
                this.$message.error('修改情感极性分析失败，请稍后再试！')
              })
          }
        } else {
          return false
      ...
      this.$confirm('是否确认删除编号为"' + newIds + '"的数据项？', '信息', {
        ...
        .then(() => {
          deletePolarity(newIds)
            ...
            .catch(err => {
              this.loading = false
              this.$message.error('删除情感极性分析失败，请稍后再试！')
            })
        })
        .catch(() => {
          this.$message({
    ...
    handleEdit: function (index, row) {
      ...
      if (row != undefined && row != 'undefined') {
        ...
        this.editForm.positive = row.positive
        this.editForm.negative = row.negative
        ...
        this.editForm.positive = null
        this.editForm.negative = null
    ...
    handleImport() {
      this.upload.title = "情感极性分析导入";
      this.upload.open = true;
    },
    handleExport() { // 处理数据导出
      this.download('/polarity/export', {}, `情感极性分析信息表${new Date().getTime()}.xlsx`)
    },
    handleExportTemplate() { // 处理导出模板
      // location.href = "http://" + "localhost" + ":9090/api/user/importTemplate";
      this.download('/polarity/importTemplate', {}, `情感极性分析信息导入模板${new Date().getTime()}.xlsx`)
    },
    // 分页插件事件
    callFather(parm) {
