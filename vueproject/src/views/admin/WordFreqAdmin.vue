<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>词频统计管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 搜索筛选 -->
    ...
    <el-table size="small" :data="list_data" highlight-current-row v-loading="loading"
      ...
      <el-table-column prop="keywords" label="高频词" width="100">
      </el-table-column>
      <el-table-column prop="polarity" label="情感极性" width="100">
      </el-table-column>
      <el-table-column prop="frequency" label="高频词词频" width="100">
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="40%" @click="closeDialog">
      <el-form label-width="130px" :model="editForm" :rules="rules" ref="editForm">
        ...
        <el-form-item label="高频词" prop="keywords">
          <el-input size="small" v-model="editForm.keywords" auto-complete="off"
                    placeholder="请输入包含的高频词"></el-input>
        </el-form-item>
        <el-form-item label="情感极性" prop="polarity">
          <el-input size="small" v-model="editForm.polarity" auto-complete="off"
                    placeholder="请输入高频词对应的情感极性"></el-input>
        </el-form-item>
        <el-form-item label="高频词词频" prop="frequency">
          <el-input size="small" v-model="editForm.frequency" auto-complete="off"
                    placeholder="请输入对应高频词词频"></el-input>
        </el-form-item>
      </el-form>
...
import {addPolarity, deletePolarity, getPolarityByPage, updatePolarity} from "../../api/polarityAPI";
import {addWordFreq, deleteWordFreq, getWordFreqByPage, updateWordFreq} from "../../api/word_freq_anaAPI";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        updateSupport: 0,
        // 上传的地址
        url: "http://localhost:9090" + "/api/word-freq/import"
      },
      editForm: {
        id: null,
        workId: null,
  ...
  methods: {
    get_data(param) {
      this.loading = true
      getWordFreqByPage(param).then(res => {
        this.loading = false
          ...
          if (this.title === '添加') {
            addWordFreq(this.editForm)
              ...
              .catch(err => {
                this.editFormVisible = false
                this.loading = false
                this.$message.error('添加词频统计失败，请稍后再试！')
              })
          } else {
            // console.log(this.editForm)
            updateWordFreq(this.editForm)
              .then(res => {
                this.editFormVisible = false
                this.loading = false
                if (res.code === "0") {
              ...
              .catch(err => {
                this.editFormVisible = false
                this.loading = false
                this.$message.error('修改词频统计失败，请稍后再试！')
              })
          }
        } else {
          return false
      ...
      this.$confirm('是否确认删除编号为"' + newIds + '"的数据项？', '信息', {
        ...
        .then(() => {
          deleteWordFreq(newIds)
            ...
            .catch(err => {
              this.loading = false
              this.$message.error('删除词频统计失败，请稍后再试！')
            })
        })
        .catch(() => {
          this.$message({
    ...
    handleEdit: function (index, row) {
      ...
      if (row != undefined && row != 'undefined') {
        ...
        this.editForm.polarity = row.polarity
        this.editForm.frequency = row.frequency
        console.log(this.editForm)
      } else {
        this.title = '添加'
        ...
        this.editForm.polarity = null
        this.editForm.frequency = null
        console.log(this.editForm)
      }
    },
    ...
    handleImport() {
      this.upload.title = "词频统计信息导入";
      this.upload.open = true;
    },
    handleExport() { // 处理数据导出
      this.download('/word-freq/export', {}, `词频统计信息表${new Date().getTime()}.xlsx`)
    },
    handleExportTemplate() { // 处理导出模板
      // location.href = "http://" + "localhost" + ":9090/api/user/importTemplate";
      this.download('/word-freq/importTemplate', {}, `词频统计信息导入模板${new Date().getTime()}.xlsx`)
    },
    // 分页插件事件
    callFather(parm) {
