  ...
  <div>
    ...
    <el-form :inline="true" :model="search_data" class="user-search">
      <el-form-item label="搜索：">
        <el-input size="small" v-model="search_data.searchUsername" placeholder="输入管理员用户名"></el-input>
      </el-form-item>
      <el-form-item label="">
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        updateSupport: 0,
        // 上传的地址
        url: "http://localhost:9090" + "/api/admin/import"
      },
      editForm: {
        id: '',
        username: '',
      ...
      rules: {
        ...
        level: [ // 管理员级别校验
          { required: true, message: '请输入管理员级别', trigger: 'blur' },
        ]
      },
      // 要删除的数据
      delete_data: {
  ...
  methods: {
          ...
          if (this.title === '添加') {
            adminRegister(this.editForm)
              ...
              .catch(err => {
                this.editFormVisible = false
                this.loading = false
                this.$message.error('添加管理员失败，请稍后再试！')
              })
          } else {
            console.log(this.editForm)
            adminUpdate(this.editForm)
              .then(res => {
                this.editFormVisible = false
                this.loading = false
                if (res.code === "0") {
              ...
              .catch(err => {
                this.editFormVisible = false
                this.loading = false
                this.$message.error('修改管理员信息失败，请稍后再试！')
              })
          }
        } else {
          return false
      ...
      this.$confirm('是否确认删除编号为"' + userIds + '"的数据项？', '信息', {
        ...
        .then(() => {
          deleteAdmin(userIds)
            ...
            .catch(err => {
              this.loading = false
              this.$message.error('删除管理员失败，请稍后再试！')
            })
        })
        .catch(() => {
          this.$message({
    ...
    // 重置用户的密码
    resetPwd(index, row) {
      this.$confirm('确定要重置该管理员的密码吗?', '信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          resetAdminPwd(row.id)
            .then(res => {
              if (res.code === '0') {
                this.$message({
                  type: 'success',
            ...
            .catch(err => {
              this.loading = false
              this.$message.error('管理员密码重置失败，请稍后再试！')
            })
        })
        .catch(() => {
          this.$message({
    ...
    handleExport() { // 处理数据导出
      this.download('/user/export', {}, `管理员信息表${new Date().getTime()}.xlsx`)
    },
    handleExportTemplate() { // 处理导出模板
      // location.href = "http://" + "localhost" + ":9090/api/user/importTemplate";
      this.download('/user/importTemplate', {}, `管理员信息导入模板${new Date().getTime()}.xlsx`)
    },
    // 分页插件事件
    callFather(parm) {
